...
def find_and_collect_posts(driver):
    ...
    for post in posts:
        ...
        time.sleep(7)
        if WebDriverWait(driver, 10).until(EC.element_to_be_clickable((By.CSS_SELECTOR, "img[style='object-fit: cover;']"))) is not None:
            download_url = WebDriverWait(driver, 10).until(EC.element_to_be_clickable((By.CSS_SELECTOR, "img[style='object-fit: cover;']"))).get_attribute('src')
            urllib.request.urlretrieve(download_url, 'images/{}.jpg'.format(shortcode))
        else:
            if not os.path.exists(video_path):
                os.makedirs(video_path)
            download_url = WebDriverWait(driver, 10).until(EC.element_to_be_clickable((By.CSS_SELECTOR, "video[type='video/mp4']"))).get_attribute('src')
